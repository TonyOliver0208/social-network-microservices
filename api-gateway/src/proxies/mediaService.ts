...
const proxyConfigs = {
  ...
  userResDecorator: (
    ...
    logger.info("Media service response", {
      requestId,
...
if (!process.env.MEDIA_SERVICE_URL) {
  throw new Error("MEDIA_SERVICE_URL environment variable is required");
}

const mediaServiceProxy = proxy(process.env.MEDIA_SERVICE_URL, proxyConfigs);

export default mediaServiceProxy;
